...
function defaultData() {
  return {
    ...
    pointagesManuels: {}   // { "NOM|2026-09-07": minutes } - corrections manuelles de la colonne Pointe
  };
}
...
function migrateData(state) {
  ...
  if (!state.pointagesManuels) { state.pointagesManuels = {}; changed = true; }
  return changed;
}
...
async function fetchAvecDelai(url, timeoutMs, options) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(`RH_USL a repondu ${res.status}${texte ? ' : ' + texte.slice(0, 150) : ''}`);
    }
...
// Render (offre gratuite) met en veille les services inactifs : le premier
// appel peut prendre 30 a 50 secondes le temps que RH_USL redemarre. On
// utilise donc un delai genereux, avec une deuxieme tentative si la
// premiere echoue (le reveil peut se terminer juste apres le timeout).
async function fetchAvecReessai(url, timeoutMs, options) {
  try {
    return await fetchAvecDelai(url, timeoutMs, options);
  } catch (e) {
...
async function getReference() {
  ...
  try {
    const [employees, settingsData] = await Promise.all([
      fetchAvecReessai(`${RH_USL_URL}/api/employees`, 25000),
      fetchAvecReessai(`${RH_USL_URL}/api/settings`, 25000)
    ]);
    cacheReference = { employees, salles: settingsData.salles || [], at: maintenant };
    return { ...cacheReference, source: 'rh-usl' };
  } catch (e) {
    console.error('Erreur lors de la recuperation de la reference RH_USL :', e.message);
    if (cacheReference.employees.length) return { ...cacheReference, source: 'cache-expiree', erreur: e.message };
...
// ---------- Detection de conflits sur une grille de semaine ----------
// 1) Doublons : un meme salarie assigne a 2 salles differentes sur le meme
//    creneau (jour + heure) -> physiquement impossible d'etre aux deux.
// 2) Ecarts contrat : total d'heures prevues cette semaine-la trop eloigne
//    (>= 2h d'ecart) des heures hebdo prevues par son contrat.
function detecterConflits(state, grille) {
  const parCreneauEtNom = {}; // "JOUR|CRENEAU" -> { NOM: Set(salles) }
  Object.keys(grille).forEach((k) => {
    const [jour, salle, creneau] = k.split('|');
    const jc = `${jour}|${creneau}`;
    nomsDeCellule(grille[k]).forEach((nom) => {
      parCreneauEtNom[jc] = parCreneauEtNom[jc] || {};
      parCreneauEtNom[jc][nom] = parCreneauEtNom[jc][nom] || new Set();
      parCreneauEtNom[jc][nom].add(salle);
    });
  });
  const doublons = [];
  Object.keys(parCreneauEtNom).forEach((jc) => {
    const [jour, creneau] = jc.split('|');
    Object.keys(parCreneauEtNom[jc]).forEach((nom) => {
      const salles = parCreneauEtNom[jc][nom];
      if (salles.size > 1) {
        doublons.push({ nom, jour, creneau: libelleCreneau(creneau), salles: [...salles] });
      }
    });
  });
  doublons.sort((a, b) => JOURS.indexOf(a.jour) - JOURS.indexOf(b.jour) || a.creneau.localeCompare(b.creneau));

  const heures = heuresParEmployeDeGrille(grille);
  const ecarts = [];
  (state.contrats || []).forEach((c) => {
    if (c.heuresHebdoContrat === null || c.heuresHebdoContrat === undefined) return;
    const prevues = heures[c.employeeNom] || 0;
    const ecart = +(prevues - c.heuresHebdoContrat).toFixed(2);
    if (Math.abs(ecart) >= 2) {
      ecarts.push({ nom: c.employeeNom, heuresPrevues: prevues, heuresContrat: c.heuresHebdoContrat, ecart });
    }
  });
  ecarts.sort((a, b) => Math.abs(b.ecart) - Math.abs(a.ecart));

  return { doublons, ecarts };
}
...
// Force un rechargement immediat des salaries/salles depuis RH_USL (ignore
// le cache de 60s) - utile pour l'onglet "Actualisation" et pour diagnostiquer
// une eventuelle panne de connexion a RH_USL.
app.post('/api/actualiser', requireAdmin, async (req, res) => {
  cacheReference = { employees: [], salles: [], at: 0 };
  const reference = await getReference();
  res.json({
    ...reference,
    jours: JOURS,
    creneaux: CRENEAUX.map((c) => ({ code: c, label: libelleCreneau(c) })),
    heureActualisation: new Date().toISOString()
  });
});

// ---------- Routes : SALARIES RH_USL (ajout/modification, en relais vers RH_USL) ----------
// Les salaries restent geres par RH_USL (source unique de verite). Ces routes
// permettent de les ajouter/modifier directement depuis Planning USL, sans
// dupliquer la liste : on se connecte a RH_USL avec le mot de passe admin de
// RH_USL fourni par l'utilisateur (jamais stocke), on recupere un token, et
// on relaie la demande.
async function tokenRhUsl(rhUslPassword) {
  const data = await fetchAvecReessai(`${RH_USL_URL}/api/admin/login`, 25000, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ password: rhUslPassword })
  });
  return data.token;
}

app.post('/api/rhusl/employees', requireAdmin, async (req, res) => {
  const { rhUslPassword, nom, couleur, heuresHebdoContrat } = req.body;
  if (!rhUslPassword) return res.status(400).json({ error: 'Mot de passe admin RH_USL requis' });
  if (!nom || !nom.trim()) return res.status(400).json({ error: 'Nom du salarie requis' });
  try {
    const token = await tokenRhUsl(rhUslPassword);
    const employee = await fetchAvecDelai(`${RH_USL_URL}/api/employees`, 15000, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Admin-Token': token },
      body: JSON.stringify({ nom, couleur: couleur || '#FFA000', heuresHebdoContrat: heuresHebdoContrat || 35 })
    });
    cacheReference = { employees: [], salles: [], at: 0 };
    res.status(201).json(employee);
  } catch (e) {
    res.status(400).json({ error: e.message.includes('401') || /incorrect/i.test(e.message) ? 'Mot de passe admin RH_USL incorrect' : e.message });
  }
});

app.put('/api/rhusl/employees/:id', requireAdmin, async (req, res) => {
  const { rhUslPassword, nom, couleur, heuresHebdoContrat } = req.body;
  if (!rhUslPassword) return res.status(400).json({ error: 'Mot de passe admin RH_USL requis' });
  try {
    const token = await tokenRhUsl(rhUslPassword);
    ...
    if (couleur !== undefined) body.couleur = couleur;
    if (heuresHebdoContrat !== undefined) body.heuresHebdoContrat = heuresHebdoContrat;
    const employee = await fetchAvecDelai(`${RH_USL_URL}/api/employees/${req.params.id}`, 15000, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', 'X-Admin-Token': token },
...
app.get('/api/planning', requireAdmin, async (req, res) => {
  ...
  res.json({
    ...
    conflits: detecterConflits(state, resultat.grille)
  });
});

// Duplique la grille effective d'une semaine source vers une semaine cible.
// Fonctionne quelle que soit la nature de la source (semaine-type, semaine
// avec exceptions, ou semaine speciale) et quelle que soit la nature de la
// cible : si la cible est une semaine speciale, on remplace sa grille ; sinon
// on cree/remplace ses exceptions ponctuelles (la semaine-type et les autres
// semaines ne sont jamais touchees).
app.post('/api/planning/dupliquer', requireAdmin, async (req, res) => {
  const state = await loadState();
  const { lundiSource, lundiCible } = req.body;
  if (!lundiSource || !lundiCible) return res.status(400).json({ error: 'Semaine source et semaine cible requises' });
  const sourceReel = mondayOf(lundiSource);
  const cibleReel = mondayOf(lundiCible);
  if (sourceReel === cibleReel) return res.status(400).json({ error: 'La semaine source et la semaine cible doivent etre differentes' });

  const { grille: grilleSource } = grilleSemaine(state, sourceReel);

  const semaineSpecialeCible = (state.semainesSpeciales || []).find((s) => s.lundi === cibleReel);
  if (semaineSpecialeCible) {
    semaineSpecialeCible.grille = { ...grilleSource };
  } else {
    // On reconstruit les exceptions de la cible pour que sa grille effective
    // (semaine-type + exceptions) reproduise exactement la grille source.
    const semaineType = state.semaineType || {};
    const toutesLesCles = new Set([...Object.keys(semaineType), ...Object.keys(grilleSource)]);
    const nouvellesExceptions = {};
    toutesLesCles.forEach((k) => {
      const voulu = grilleSource[k] || '';
      const parDefaut = semaineType[k] || '';
      if (voulu !== parDefaut) nouvellesExceptions[k] = voulu; // '' = supprime explicitement la valeur de la semaine-type
    });
    state.exceptions = state.exceptions || {};
    if (Object.keys(nouvellesExceptions).length) state.exceptions[cibleReel] = nouvellesExceptions;
    else delete state.exceptions[cibleReel];
  }

  await saveState(state);
  const resultat = grilleSemaine(state, cibleReel);
  res.json({
    lundi: cibleReel,
    dimanche: addDays(cibleReel, 6),
    numeroSemaine: numeroSemaineISO(cibleReel),
    source: resultat.source,
    semaineSpeciale: resultat.semaineSpeciale || null,
    grille: resultat.grille,
    heuresParEmploye: heuresParEmployeDeGrille(resultat.grille),
    conflits: detecterConflits(state, resultat.grille)
  });
});
...
app.get('/api/semaines-speciales', requireAdmin, async (req, res) => {
  const state = await loadState();
  const liste = [...(state.semainesSpeciales || [])]
    .sort((a, b) => a.lundi.localeCompare(b.lundi))
    .map((s) => ({ ...s, numeroSemaine: numeroSemaineISO(s.lundi) }));
  res.json(liste);
});
...
// Petit resume automatique (mois en cours + annee en cours, tous salaries
// confondus) affiche en haut du Tableau de bord sans avoir besoin de
// choisir une periode.
app.get('/api/tableau-bord/resume', requireAdmin, async (req, res) => {
  const state = await loadState();
  const maintenant = new Date();
  const moisCle = formatDateLocal(maintenant).slice(0, 7);
  const anneeCle = String(maintenant.getFullYear());
  const debutMois = `${moisCle}-01`;
  const finMois = formatDateLocal(new Date(maintenant.getFullYear(), maintenant.getMonth() + 1, 0));
  const debutAnnee = `${anneeCle}-01-01`;
  const finAnnee = `${anneeCle}-12-31`;

  function totalPeriode(from, to) {
    let total = 0;
    let lundiCourant = mondayOf(from);
    let garde = 0;
    while (lundiCourant <= to && garde < 60) {
      ...
      Object.keys(grille).forEach((k) => {
        const [jour] = k.split('|');
        const dateJour = addDays(lundiCourant, JOURS.indexOf(jour));
        if (dateJour < from || dateJour > to) return;
        total += nomsDeCellule(grille[k]).length;
      });
      lundiCourant = addDays(lundiCourant, 7);
    }
    return total;
  ...
  res.json({
    mois: { cle: moisCle, total: totalPeriode(debutMois, finMois) },
    annee: { cle: anneeCle, total: totalPeriode(debutAnnee, finAnnee) }
  });
});

// ---------- Route : RECAP HEURES (par semaine / mois / annee, pour un salarie) ----------

app.get('/api/recap-heures', requireAdmin, async (req, res) => {
  const state = await loadState();
  const { employeeNom } = req.query;
  if (!employeeNom) return res.status(400).json({ error: 'Salarie requis' });
  const cible = employeeNom.trim().toUpperCase();
  const from = req.query.from ? mondayOf(req.query.from) : state.settings.debutSaison;
  const to = req.query.to || state.settings.finSaison;
  ...
  while (lundiCourant <= to && garde < 60) {
    ...
    Object.keys(grille).forEach((k) => {
      const [jour] = k.split('|');
      if (!nomsDeCellule(grille[k]).includes(cible)) return;
      totalSemaine += 1;
      const dateJour = addDays(lundiCourant, JOURS.indexOf(jour));
      const moisCleJour = dateJour.slice(0, 7);
      const anneeCleJour = dateJour.slice(0, 4);
      parMois[moisCleJour] = (parMois[moisCleJour] || 0) + 1;
      parAnnee[anneeCleJour] = (parAnnee[anneeCleJour] || 0) + 1;
    });
    if (totalSemaine > 0) parSemaine[lundiCourant] = totalSemaine;
    lundiCourant = addDays(lundiCourant, 7);
  }

  const semaines = Object.keys(parSemaine).sort().map((lundi) => ({
    lundi, numeroSemaine: numeroSemaineISO(lundi), total: parSemaine[lundi]
  }));
  const mois = Object.keys(parMois).sort().map((m) => ({ mois: m, total: parMois[m] }));
  const annees = Object.keys(parAnnee).sort().map((a) => ({ annee: a, total: parAnnee[a] }));

  res.json({ employeeNom: cible, from, to, semaines, mois, annees });
});
...
app.get('/api/verification-heures', requireAdmin, async (req, res) => {
  ...
  let statutRhUsl = 'ok';
  try {
    const data = await fetchAvecReessai(`${RH_USL_URL}/api/public/heures?from=${from}&to=${to}`, 25000);
    heuresPointees = (data.heures && data.heures[cible]) || {};
  } catch (e) {
    statutRhUsl = 'indisponible';
  }

  const overrides = state.pointagesManuels || {};
  ...
  while (dateCourante <= to && garde < 400) {
    ...
    const cleOverride = `${cible}|${dateCourante}`;
    const aOverride = Object.prototype.hasOwnProperty.call(overrides, cleOverride);
    const pointeesMin = aOverride ? overrides[cleOverride] : (heuresPointees[dateCourante] || 0);
...
// Correction manuelle de la colonne "Pointe" pour un jour precis (par
// exemple si RH_USL n'a pas la bonne valeur). heures vide/null = on
// supprime la correction et on revient a la valeur RH_USL.
app.put('/api/verification-heures/pointage-manuel', requireAdmin, async (req, res) => {
  const state = await loadState();
  const { employeeNom, date, heures } = req.body;
  if (!employeeNom || !date) return res.status(400).json({ error: 'Salarie et date requis' });
  const cible = employeeNom.trim().toUpperCase();
  const cle = `${cible}|${date}`;
  state.pointagesManuels = state.pointagesManuels || {};
  if (heures === '' || heures === null || heures === undefined) {
    delete state.pointagesManuels[cle];
  } else {
    const minutes = Math.round(Number(heures) * 60);
    if (Number.isNaN(minutes) || minutes < 0) return res.status(400).json({ error: 'Heures invalides' });
    state.pointagesManuels[cle] = minutes;
  }
  await saveState(state);
  res.json({ ok: true, pointeesHeures: state.pointagesManuels[cle] !== undefined ? +(state.pointagesManuels[cle] / 60).toFixed(2) : null });
});
